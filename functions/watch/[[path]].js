export async function onRequest(context) {
  ...
  // Lista de bots/crawlers comuns que queremos interceptar
  const isBot = /WhatsApp|facebookexternalhit|Twitterbot|Slackbot|Discordbot|TelegramBot/i.test(userAgent);
  ...
  try {
    ...
    const type = parts[2]; // 'filme' ou 'serie'
    const slug = parts[3];
    
    // URL da API baseada no config.js (produção)
    const API_BASE = "https://pipoca-backend-jazs.onrender.com/api";
    const infoUrl = `${API_BASE}/${type}/${slug}`;
    
    // Busca dados do filme/série na API
    const response = await fetch(infoUrl);
    if (!response.ok) return await next();
    
    const data = await response.json();
    
    // Pega o HTML original do index.html
    ...
    // Substitui o título padrão e injeta as novas tags
    html = html.replace('<title>Pipoca Filmes</title>', metaTags);
